...
class Crawler:
    def __init__(self, symbol, output_file):
        ...
        self.ws = websocket.WebSocketApp('wss://api.gemini.com/v1/marketdata/{}'.format(symbol),
                                ...
                                # 使用lambda匿名函数，传递只需要的message
                                # 因为self.on_message是一个类成员函数，第一个参数是self，而非ws
...
# 设计思路：
# 封装了 orderbook的class， 专门用来存放与之相关的数据结构。
# crawler类：构造函数声明 orderbook，然后定义 Websocket 用来接收交易数据。  
